...
async function uploadBatch(){const nid=document.getElementById('bNovelaId').value;if(!nid){toast('Selecione uma novela','err');return;}if(!BATCH_FILES.length){toast('Selecione os vídeos','err');return;}const btn=document.getElementById('batchBtn');btn.disabled=true;btn.textContent='Enviando em lote...';document.getElementById('bpw').style.display='block';document.getElementById('bpf').style.width='0%';const startEp=parseInt(document.getElementById('bStartEp').value)||1;const coinCost=document.getElementById('bCoins').value;let ok=0,errCount=0;for(let idx=0;idx<BATCH_FILES.length;idx++){const f=BATCH_FILES[idx];const p=parseFilename(f.name);const ep=p.ep>0?p.ep:startEp+idx;const epTitle=p.title||('Episódio '+ep);const el=document.getElementById('bst-'+idx);const bi=document.getElementById('bi-'+idx);if(el){el.textContent='⬆️ enviando...';el.style.color='var(--gold)';}document.getElementById('bpt').textContent=(idx+1)+' / '+BATCH_FILES.length+' — '+f.name;const fd=new FormData();fd.append('novela_id',nid);fd.append('episode_title',epTitle);fd.append('episode_number',ep);fd.append('description','');fd.append('coin_cost',coinCost);fd.append('thumbnail_url','');fd.append('duration_seconds','0');fd.append('video',f);const result=await new Promise(resolve=>{const xhr=new XMLHttpRequest();xhr.upload.onprogress=ev=>{if(ev.lengthComputable){const fp=ev.loaded/ev.total;const tp=(idx+fp)/BATCH_FILES.length;document.getElementById('bpf').style.width=Math.round(tp*100)+'%';document.getElementById('bpt').textContent=(idx+1)+' / '+BATCH_FILES.length+' — '+Math.round(fp*100)+'%';}};xhr.onload=()=>{if(xhr.status===200){let d={};try{d=JSON.parse(xhr.responseText);}catch(e){}resolve({ok:true,data:d});}else{let msg='Erro '+xhr.status;try{msg=JSON.parse(xhr.responseText).detail||msg;}catch(e){}resolve({ok:false,msg});}};xhr.onerror=()=>resolve({ok:false,msg:'Erro de rede'});xhr.open('POST',API+'/api/admin/upload');xhr.setRequestHeader('X-Admin-Token',TOKEN);xhr.send(fd);});if(result.ok){ok++;if(el){el.textContent='✅ '+fmtDur(result.data.durationSeconds||0);el.style.color='var(--green)';}if(bi)bi.classList.add('done');}else{errCount++;if(el){el.textContent='❌ '+result.msg;el.style.color='var(--red)';}if(bi)bi.classList.add('err');}}btn.disabled=false;btn.textContent='📦 Enviar Todos';document.getElementById('bpf').style.width='100%';document.getElementById('bpt').textContent='Concluído: '+ok+' ok'+(errCount?', '+errCount+' erro(s)':'');toast(ok+' episódio(s) enviado(s)'+(errCount?' | '+errCount+' erro(s)':''),errCount?'err':'ok');loadEps();loadStats();}
